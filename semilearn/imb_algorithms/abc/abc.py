...
class ABC(ImbAlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        ...
        self.ema_model.load_state_dict(self.model.state_dict()) #包含模型所有可学习参数的字典
    ...
    def process_batch(self, **kwargs):  #process batch data, send data to cuda
    ...
    def train_step(self, *args, **kwargs):
        ...
        # get features
        feats_x_lb = out_dict['feat']['x_lb']   #见fixmatch train_step
    ...
    @torch.no_grad()
    def bernouli_mask(x):
        return torch.bernoulli(x.detach()).float()  #detach 得到的张量没有梯度
    ...
    def compute_abc_loss(self, logits_x_lb, y_lb, logits_x_ulb_w, logits_x_ulb_s):
        ...
        abc_lb_loss = (self.ce_loss(logits_x_lb, y_lb, reduction='none') * mask_lb).mean()  #L_cls

        # compute unlabeled abc loss
        with torch.no_grad():
            # probs_x_ulb_w = torch.softmax(logits_x_ulb_w, dim=1)
            probs_x_ulb_w = self.compute_prob(logits_x_ulb_w)   #softmax
        ...
        for logits_s in logits_x_ulb_s:
            abc_ulb_loss += (self.ce_loss(logits_s, y_ulb, reduction='none') * mask_ulb).mean() #L_con
